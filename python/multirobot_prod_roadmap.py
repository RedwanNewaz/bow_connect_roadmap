...
robot2_start = [-1.5, -1.00375, 0.785398]
robot2_goal = [-11.5, -11.00375]
...
def draw_roadmap(viz, roadmap, path=None, output=None):
    ...
    if 'map_image_path' in config:
        ...
        robot2_start_ = to_img(robot2_start[:2])[0]
        robot2_goal_ = to_img(robot2_goal[:2])[0]
        ax.grid(False)
    else:
        pos = pos_world
    ...
    for p in [path, roadmap.shortest_path(robot2_start, robot2_goal)]:
        if path is not None:
            pxy = np.array([pos[n] for n in p])
    ...
    ax.scatter(robot2_start_[0], robot2_start_[1], c='orange', s=200, zorder=6, label='Robot 2 Start')
    ax.scatter(robot2_goal_[0], robot2_goal_[1], c='purple', s=200, zorder=6, label='Robot 2 Goal')
    ax.set_aspect('equal')
    ax.legend()
...
def main():
    ...
    for i in range(args.num_plans):
        try:
            ...
            traj2 = planner.plan(
                start=robot2_start,               # [x, y, theta] from the config
                goal=robot2_goal,             # [x, y] from the config
                solver_time=args.solver_time,
                optimize=False,
            )
